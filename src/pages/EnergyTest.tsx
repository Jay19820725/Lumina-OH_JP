      ...
      <div className="w-full space-y-20 md:space-y-32">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-32">
          <motion.div variants={itemVariants} className="space-y-8">
            <div className="flex items-center justify-center gap-4">
              ...
              <h3 className="text-[10px] uppercase tracking-[0.6em] text-ink-muted">{t('test.images_label')}</h3>
              <div className="h-px w-8 bg-ink/5" />
            </div>
          ...
          <motion.div variants={itemVariants} className="space-y-8">
            <div className="flex items-center justify-center gap-4">
              <div className="h-px w-8 bg-ink/5" />
              <h3 className="text-[10px] uppercase tracking-[0.6em] text-ink-muted">{t('test.words_label')}</h3>
              <div className="h-px w-8 bg-ink/5" />
            </div>
            <div className="grid grid-cols-3 gap-4 md:gap-6">
              {words.map((card) => {
        ...
        <motion.div variants={itemVariants} className="w-full space-y-12">
          <div className="flex items-center justify-center gap-6">
            ...
            <span className="text-[10px] uppercase tracking-[0.8em] text-ink-muted whitespace-nowrap">{t('test.resonance_label')}</span>
            <div className="h-px flex-1 bg-ink/5" />
          </div>
        ...
        {isAllPaired && (
          ...
          >
            <Button onClick={handleFinish} className="h-20 px-16 gap-4 text-lg shadow-2xl shadow-emerald-900/20 bg-emerald-500 hover:bg-emerald-600">
              {t('test.confirm_pairing')} <ArrowRight size={20} />
            </Button>
          </motion.div>
        )}
      </AnimatePresence>
  ...
  return (
    ...
    >
          ...
          >
            ...
            <div className="space-y-3 md:space-y-4">
              <div className="flex justify-between items-end">
                <span className="text-[10px] uppercase tracking-widest text-ink-muted">{t('test.association_label')} {i + 1}</span>
              ...
              <textarea
                ...
                placeholder={t('test.association_placeholder')}
      ...
      >
        ...
        >
          {t('test.view_results')} <ArrowRight size={18} />
        </Button>
      </motion.div>
    </motion.div>
  );
      ...
      <div className="w-full flex flex-col md:flex-row justify-between items-start md:items-end mb-16 md:mb-24 gap-8 md:gap-20 px-4">
        ...
        >
          <motion.div variants={itemVariants} className="flex items-center gap-4">
            <span className="text-[10px] uppercase tracking-[0.6em] text-ink-muted">
              {drawStage === 'revealed' ? t('test.step_final') : 
               drawStage === 'associating' ? t('test.step_4') :
               drawStage === 'pairing' ? t('test.step_3') :
               drawStage === 'drawing_words' ? t('test.step_2') : t('test.step_1')}
            </span>
          ...
          <motion.h1 variants={itemVariants} className="text-4xl md:text-6xl lg:text-7xl font-serif tracking-[0.05em] leading-[1.1] max-w-2xl">
            {drawStage === 'drawing_images' ? t('test.stage_images_title') : 
             drawStage === 'drawing_words' ? t('test.stage_words_title') : 
             drawStage === 'pairing' ? t('test.stage_pairing_title') :
             drawStage === 'associating' ? t('test.stage_associating_title') :
             drawStage === 'revealed' ? t('test.stage_revealed_title') : t('test.stage_idle_title')}
          </motion.h1>
          <motion.p variants={itemVariants} className="text-sm md:text-base text-ink-muted max-w-lg font-light leading-[1.8] tracking-wide opacity-80">
            {drawStage === 'drawing_images' ? t('test.stage_images_desc') : 
             drawStage === 'drawing_words' ? t('test.stage_words_desc') : 
             drawStage === 'pairing' ? t('test.stage_pairing_desc') :
             drawStage === 'associating' ? t('test.stage_associating_desc') :
             drawStage === 'revealed' ? t('test.stage_revealed_desc') : 
             t('test.stage_idle_desc')}
          </motion.p>
        </motion.div>
        ...
        >
          <div className="text-center md:text-right">
            <span className="text-[10px] uppercase tracking-[0.4em] text-ink-muted block mb-2 md:mb-4">{t('test.selected_label')}</span>
          ...
          {drawStage === 'shuffling' ? (
              ...
              {isDrawing && (
                ...
                >
                  <div className="flex flex-col items-center gap-6">
                    ...
                    <span className="text-[11px] uppercase tracking-[0.6em] text-ink/40 font-light">{t('test.syncing_energy')}</span>
                  </div>
                </motion.div>
              )}
            </motion.div>
            ...
            >
              <motion.div variants={itemVariants} className="text-center space-y-2 mb-8 md:mb-12">
                <h2 className="text-xl md:text-2xl font-serif">{t('test.ready_title')}</h2>
                <p className="text-xs md:text-sm text-ink-muted">{t('test.ready_desc')}</p>
              </motion.div>
                ...
                {selectedCards.pairs?.map((pair, i) => (
                  ...
                  >
                   ...
                   <div className="text-center space-y-1 md:space-y-2">
                      <span className="text-[8px] uppercase tracking-widest text-ink-muted">{t('test.association_label')}</span>
